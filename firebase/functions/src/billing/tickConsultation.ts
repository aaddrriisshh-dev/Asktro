...
export async function applyTick(
  ...
  // AI consultations are billed by the SAME metering as human chats below. AI has
  // no astrologer heartbeat, so `astroTracked` stays false and the meter runs on
  // the customer's presence alone; the free opening is funded by chatBonusBalance
  // (the chat welcome credit), not by skipping the charge.
