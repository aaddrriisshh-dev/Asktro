    ...
    try {
      ...
      engine.registerEventHandler(RtcEngineEventHandler(
        ...
        onUserJoined: (RtcConnection connection, int uid, int elapsed) {
          ...
          remoteJoined = true;
          phase = CallPhase.connected;
          errorMessage = null; // clear any transient "connecting" notice
          notifyListeners();
        },
        onUserOffline:
        ...
        // Surface only a GENUINE, fatal failure; clear any stale notice the
        // moment the connection is established. Normal "connecting" is conveyed
        // by the screen's own "Connecting…" state (call.isLive) — never an error
        // line, otherwise a perfectly live call looks stuck (the bug we hit while
        // diagnosing). onConnectionLost is a transient blip the SDK auto-recovers
        // from and the authoritative end stays server-driven (billing), so we do
        // not tear down or alarm on it; other onError codes are logged only.
